...
\end{equation}
Again the Hamiltonian may be diagonalized by setting $U^{\dagger}(\rbf,t)H(\rbf)U(\rbf,t) = H_{\kbf}$, or equally $H(\rbf)U(\rbf,t) = U(\rbf,t)H_{\kbf}$, where $H_{\kbf}$ is on the form given in equation \eqref{H_diag}. Note that $E_{\kbf}$ is in general as defined for plane waves in equation \eqref{Ek}. %and $E_{\kbf}$ is solution to the eigenvalue equation $i\hbar \frac{\partial \psi(\rbf,t)}{\partial t} = E_{\kbf} \psi(\rbf,t)$. 
